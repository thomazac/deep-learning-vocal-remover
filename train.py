...
def train_epoch(dataloader, model, device, optimizer, accumulation_steps):
    ...
    sum_loss = 0
    crit = nn.L1Loss()
    data_length = len(dataloader.dataset)  # Keep a reference to the dataset length
    
    # Wrap dataloader with tqdm for a progress bar
    progress_bar = tqdm(dataloader, desc="Training")

    for itr, (X_batch, y_batch) in enumerate(progress_bar):
    ...
    return sum_loss / data_length  # Use the saved dataset length
...
def validate_epoch(dataloader, model, device):
    ...
    with torch.no_grad():
        for X_batch, y_batch in progress_bar:
            ...
            y_batch = spec_utils.crop_center(y_batch, pred)  # Assuming spec_utils is a module you have access to
